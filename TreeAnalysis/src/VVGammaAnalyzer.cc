...
void VVGammaAnalyzer::analyze(){
  ...
  //   }
  // }
        if( (region_==CR3P1F || region_==CR2P2F || region_==SR4P ) && ZZ && ZZ->pt() > 1.){
	        theHistograms->fill("ZZ mass", "m_{4l} ;GeV/c^{2}",  25,0.,500.,  ZZ->mass(),                    theWeight);
		theHistograms->fill("Z0 mass", "m_{Z0} ;GeV/c^{2}",  35,55.,125., ZZ->first(). mass(),           theWeight);
		theHistograms->fill("Z1 mass", "m_{Z1} ;GeV/c^{2}",  35,55.,125., ZZ->second().mass(),           theWeight);
		theHistograms->fill("Z0_l0_pt", "pT_{l0,Z0} ;GeV/c", 20,0.,400.,  ZZ->first(). daughter(0).pt(), theWeight);
		theHistograms->fill("Z0_l1_pt", "pT_{l1,Z0} ;GeV/c", 20,0.,400.,  ZZ->first(). daughter(1).pt(), theWeight);
		theHistograms->fill("Z1_l0_pt", "pT_{l0,Z1} ;GeV/c", 20,0.,400.,  ZZ->second().daughter(0).pt(), theWeight);
	        theHistograms->fill("Z1_l1_pt", "pT_{l1,Z1} ;GeV/c", 20,0.,400.,  ZZ->second().daughter(1).pt(), theWeight);
	    }
	if( ((region_>=CR110 && region_<=CR000) || region_==SR3P ) && ZW && ZW->pt() > 1.){
		...
		theHistograms->fill("Z_l0_pt",  "pT_{l0,Z} ;GeV/c"   , 20,0.,400.,  ZW->first(). daughter(0).pt(), theWeight);
		theHistograms->fill("Z_l1_pt",  "pT_{l1,Z} ;GeV/c"   , 20,0.,400.,  ZW->first(). daughter(1).pt(), theWeight);
		theHistograms->fill("W_l_pt",   "pT_{l,W} ;GeV/c"    , 20,0.,400.,  ZW->second().daughter(0).pt(), theWeight);
		theHistograms->fill("W_MET_pt", "MET ;GeV/c"         , 20,0.,400.,  ZW->second().daughter(1).pt(), theWeight);
	}
	if(region_==CRLFR && ZL && ZL->first.pt() > 1.){
		...
		theHistograms->fill("Z_l0_pt", "pT_{l0,Z} ;GeV/c" , 20,0.,400.,  ZL->first.daughter(0).pt(),          theWeight);
		theHistograms->fill("Z_l1_pt", "pT_{l1,Z} ;GeV/c" , 20,0.,400.,  ZL->first.daughter(1).pt(),          theWeight);
		theHistograms->fill("L_pt",    "pT_{l} ;GeV/c"    , 20,0.,400.,  ZL->second.pt(),                     theWeight);
	}
